...
const fillPassengerForm = (passenger, passengerIndex) => {
    ...
    if (treatmentDropdown) {
        treatmentDropdown.innerText = passenger.tratamiento;
    }
    // Nombre
    const nameInput = document.querySelector(`[data-ref="pax-details__name"][data-ref*="${baseRef}"] input`);
    if (nameInput) {
        nameInput.value = passenger.nombre;
    }
    // Primer apellido
    const firstSurnameInput = document.querySelector(`[data-ref="pax-details__first_surname"][data-ref*="${baseRef}"] input`);
    if (firstSurnameInput) {
        firstSurnameInput.value = passenger.primer_apellido;
    }
    // Segundo apellido
    const secondSurnameInput = document.querySelector(`[data-ref="pax-details__second_surname"][data-ref*="${baseRef}"] input`);
    if (secondSurnameInput) {
        secondSurnameInput.value = passenger.segundo_apellido;
    }
    // Checkbox "No tengo segundo apellido"
    const noSecondSurnameCheckbox = document.querySelector(`[data-ref="pax-details__second_surname_check"][data-ref*="${baseRef}"] input`);
    if (noSecondSurnameCheckbox) {
        noSecondSurnameCheckbox.checked = !passenger.segundo_apellido_bool;
    }
    // Toggle de asistencia especial
    const specialAssistanceToggle = document.querySelector(`[data-ref^="sa-activate-toggle-${baseRef}"] input`);
